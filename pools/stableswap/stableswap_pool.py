...
class StableSwap():
    ...
    def get_dy(self, i, j, dx):
        """
        Quote an exchange between two coins.
        Index values can be found via the `coins` public getter method.
    ...
    def quote_price(self, i, j, dx):
        """
        Returns the an estimated trade price of i-th coin quoted in terms of j-th coin,
        i.e. the ratio of output coin amount to input coin amount for a trade of dx size.
        ...
        dy, fee = self.get_dy(i, j, dx)
        return float(dy/dx*(self.rates[j]/self.rates[i]))
